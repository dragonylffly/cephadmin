 ...
 */
int64_t BitMapArea::get_zone_size(Config *conf)
{
  return conf->blocks_per_zone;
}

int64_t BitMapArea::get_span_size(Config *conf)
{
...
class BitMapAllocator : public Allocator {
  ...
  int64_t allocate_dis(
    ...
    int64_t hint, vector<AllocExtent> *extents);

public:
  BitMapAllocator(Config *conf, int64_t device_size, int64_t block_size);
  ...
  int64_t allocate(
    uint64_t want_size, uint64_t alloc_unit, uint64_t max_alloc_size,
    int64_t hint, vector<AllocExtent> *extents) override;
...
BitMapAllocator::BitMapAllocator(Config *conf, int64_t device_size,
				 int64_t block_size)
  : cct(cct)
{
  if (!ISP2(block_size)) {
    cerr << __func__ << " block_size " << block_size
  ...
  int64_t zone_size_blks = conf->blocks_per_zone;
  if (!ISP2(zone_size_blks)) {
    cerr << __func__ << " zone_size " << zone_size_blks
         << " not power of 2 aligned!"
         << endl;
  ...
  int64_t span_size = conf->span_size;
  if (!ISP2(span_size)) {
    cout << __func__ << " span_size " << span_size
         << " not power of 2 aligned!"
         << endl;
  ...
  cout << __func__ << " instance " << (uint64_t) this
...
{
  cout << __func__ << " instance " << this << endl;
  m_bit_alloc->dump();
}
...
{
  cout << __func__ << " instance " << (uint64_t) this << endl;
